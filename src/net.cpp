...
{
    while (!v2_priv_key.IsValid()) {
        v2_priv_key.MakeNewKey(true);
        ellswift_pubkey = v2_priv_key.EllSwiftEncode().value();
    }
}
...
void CConnman::DowngradeToV1Transport(CNode& node)
{
    ...
    node.v2_key_exchange_complete = true;
}
        ...
        if (recvSet || errorSet)
        {
            ...
            if (nBytes > 0) {
                ...
                if (pnode->PreferV2Conn() &&
                    !pnode->IsInboundConn() &&
                    (pnode->peer_ellswift_buf.empty() && !pnode->v2_keys_derived)) {
...
void CConnman::PushV2EllSwiftPubkey(CNode* pnode)
{
    ...
    memcpy(ellswift_bytes.data(), pnode->ellswift_pubkey.data(), ELLSWIFT_ENCODED_SIZE);
    size_t nBytesSent;
    {
        LOCK(pnode->cs_vSend);
