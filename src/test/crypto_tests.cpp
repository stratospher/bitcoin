...
static void TestChaCha20Poly1305AEAD(bool must_succeed, unsigned int expected_aad_length, const std::string& hex_m, const std::string& hex_k1, const std::string& hex_k2, const std::string& hex_encrypted_message, const std::string& hex_encrypted_message_seq_999)
{
    ...
    const std::vector<unsigned char> original_plaintext_bytes = ParseHex(hex_m);
    std::vector<unsigned char> plaintext_buf = original_plaintext_bytes;
    ...
    ChaCha20Poly1305AEAD aead_out(aead_K_1, aead_K_2);
    ChaCha20Poly1305AEAD aead_in(aead_K_1, aead_K_2);
    ...
    auto packet_length = htole32(expected_aad_length);
    memcpy(plaintext_buf.data(), &packet_length, 3);

    BOOST_CHECK_EQUAL(0, memcmp(plaintext_buf.data(), original_plaintext_bytes.data(), original_plaintext_bytes.size()));
    ...
    for (size_t i = 0; i < 1000; ++i) {
        ...
        // length is not decrypted, copy it over
        packet_length = htole32(expected_aad_length);
        memcpy(plaintext_buf_new.data(), &packet_length, 3);

        // make sure we always get the same plaintext
        BOOST_CHECK_EQUAL(0, memcmp(plaintext_buf_new.data(), original_plaintext_bytes.data(), original_plaintext_bytes.size()));
    }

    // compare at final value against the test vector
...
BOOST_AUTO_TEST_CASE(chacha20_poly1305_aead_testvector)
{
    ...
    TestChaCha20Poly1305AEAD(true, 252,
                             ...
                             "");
}
