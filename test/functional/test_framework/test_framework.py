...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def connect_nodes(self, a, b, use_p2p_v2=False):
        from_connection = self.nodes[a]
        to_connection = self.nodes[b]
        ip_port = "127.0.0.1:" + str(p2p_port(b))
        if use_p2p_v2:
            from_connection.addnode(ip_port, "onetry", use_p2p_v2)
        else:
        ...
        wait_until_helper(lambda: all(peer['bytesrecv_per_msg'].pop('verack', 0) in [20, 24] for peer in from_connection.getpeerinfo()))
        wait_until_helper(lambda: all(peer['bytesrecv_per_msg'].pop('verack', 0) in [20, 24] for peer in to_connection.getpeerinfo()))
