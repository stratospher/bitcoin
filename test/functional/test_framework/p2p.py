...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor, supports_v2_p2p=False):
        ...
        if supports_v2_p2p:
            ...
            self.v2_state = EncryptedP2PState(initiating=True)
    ...
    def v2_handshake(self):
        ...
        if self.v2_state.peer:
            ...
            if len(self.recvbuf) < 16+20+20:
                return
            length, is_mac_auth = self.v2_state.authenticate_handshake(BytesIO(self.recvbuf))
            if not is_mac_auth:
...
class P2PInterface(P2PConnection):
    ...
    def wait_for_reconnect(self, timeout=60):
        self.wait_for_connect(timeout)  # wait for connection using v2
        self.wait_for_disconnect(timeout)  # wait for disconnection from v2
        self.wait_for_connect(timeout)  # wait for connection using v1
