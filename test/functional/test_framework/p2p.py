...
class P2PConnection(asyncio.Protocol):
    ...
    def connection_made(self, transport):
        ...
        if self.support_v2_p2p and not self.v2_connection.tried_v2_handshake:
            # in an inbound connection, p2p connection being the initiator initiates the v2 handshake
    ...
    def v2_handshake(self):
        """initial v2 handshake is performed (see BIP 324)"""
        if self.v2_connection.initiating:
    ...
    def _on_data(self):
        ...
        try:
            while True:
                if self.support_v2_p2p:
                    if not self.v2_connection.tried_v2_handshake:
                        self.v2_handshake()
                    ...
                    if 0 < size_or_shortid <= 12:
                        ...
                        msg = msg[1+size_or_shortid:] # msg is set to be payload
                    else:
                        # a short id
    ...
    def build_message(self, message, is_decoy=False):
        ...
        if self.support_v2_p2p:
            ...
            if msgtype in MESSAGEMAP:
                ...
                tmsg = len(msgtype).to_bytes(1, 'big')
                tmsg += msgtype
            tmsg += data
